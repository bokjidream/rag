...
class StepExecutor:
    ...
    def _extract_ac_commands(self, step_num: int) -> list:
        """step 파일의 ## Acceptance Criteria 섹션에서 실행 가능한 커맨드를 추출한다."""
        ...
        for line in match.group(1).split("\n"):
            cmd = line.split("#")[0].strip()  # 인라인 주석 제거
            if cmd and not cmd.startswith("{"):  # 미입력 플레이스홀더 건너뜀
                commands.append(cmd)
        return commands
    ...
    def _verify_ac(self, step_num: int) -> tuple:
        ...
        for cmd in commands:
            ...
            if result.returncode != 0:
                ...
                return False, f"`{cmd}` 실패 (exit {result.returncode}):\n{output}"
        return True, ""
    ...
    def _build_preamble(self, guardrails: str, step_context: str,
        ...
        return (
            ...
            f"## TDD 작업 순서 (MANDATORY — 반드시 이 순서를 지켜라)\n\n"
            f"1. [RED]      테스트 코드를 먼저 작성한다. 구현 코드보다 테스트가 항상 선행한다.\n"
            f"2. [RED]      테스트를 실행하여 실패하는지 확인한다. 통과하면 테스트가 잘못된 것이다.\n"
            f"3. [GREEN]    테스트를 통과하는 최소 구현 코드를 작성한다.\n"
            f"4. [GREEN]    테스트를 실행하여 모든 테스트가 통과함을 확인한다.\n"
            f"5. [REFACTOR] 코드를 개선한다. 테스트는 계속 통과해야 한다.\n\n"
            f"## 작업 규칙\n\n"
    ...
    def _execute_single_step(self, step: dict, guardrails: str) -> bool:
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            ...
            if status == "completed":
                ac_passed, ac_error = self._verify_ac(step_num)
                if not ac_passed:
                    for s in index["steps"]:
                        if s["step"] == step_num:
                            s["status"] = "pending"
                    ...
                    prev_error = (
                        f"하네스 AC 독립 검증 실패 "
                        f"(Claude가 completed로 마킹했으나 커맨드가 실패함):\n{ac_error}"
                    )
                    print(f"  ✗ Step {step_num}: AC 독립 검증 실패 [{elapsed}s] — 재시도")
                    continue
