...
### ADR-009: 복지로 파일 다운로드 — 공개 메타데이터와 사용자 인증 파일 분리

**결정**: `application_forms`의 URL은 공공데이터포털 상세 API가 제공하는 서식/자료 메타데이터로만 저장한다. RAG 서버는 복지로 사용자 로그인 세션, 본인인증, 쿠키, CSRF 토큰이 필요한 파일을 대리 다운로드하지 않는다.

**이유**: 복지로 서비스신청 영역은 사회복지서비스 신청, 온라인 신청 상담, 복지대상자 선정 및 확인을 위한 개인정보를 처리한다. 사용자 인증이 필요한 파일을 서버가 대신 받으려면 사용자의 인증정보나 세션을 취급해야 하므로 개인정보·보안·약관 리스크가 커진다.

**처리 방식**: 공개 API로 조회 가능한 안내 정보와 서식 메타데이터는 배치 수집한다. 실제 URL 접근 시 복지로 세션이 필요한 파일은 사용자 브라우저에서 복지로로 이동해 직접 인증/다운로드하게 하고, 우리 서비스에는 사용자가 내려받은 파일을 업로드하는 흐름으로 처리한다. 기관 연계 API나 명시적 계약이 생기면 별도 ADR로 서버 간 연계를 재검토한다.

**트레이드오프**: UX는 한 단계 늘어나지만, 사용자 인증정보를 보관하거나 프록시하지 않아 보안 경계가 명확해진다.
